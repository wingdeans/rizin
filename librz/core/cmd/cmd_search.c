...
static int _cb_hit(RzSearchKeyword *kw, void *user, ut64 addr) {
	...
	int keyword_len = kw ? kw->keyword_length + (search->params->mode == RZ_SEARCH_DELTAKEY) : 0;
...
static void do_string_search(RzCore *core, struct search_parameters *param) {
	...
	if (param->inverse) {
		core->search->params->search_maxhits = 1;
	}
	if (core->search->params->n_kws > 0) {
		...
		rz_list_foreach (param->boundaries, iter, map) {
			...
			for (at = from1; at != to1; at = search->params->backwards ? at - len : at + len) {
				...
				if (core->search->params->search_maxhits > 0 && core->search->nhits >= core->search->params->search_maxhits) {
					goto done;
				}
			}
			print_search_progress(at, to1, search->nhits, param);
...
static bool setup_params(RzCore *core, struct search_parameters *param) {
	...
	core->search->params->search_overlap = rz_config_get_i(core->config, "search.overlap");
